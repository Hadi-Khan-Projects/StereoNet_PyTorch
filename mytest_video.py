...
def depth_to_pcd(depth, color_img):
    ...
    pcd.points = o3d.utility.Vector3dVector(np.column_stack([x.flat, y.flat, depth.flat]))
    # pcd.points = o3d.utility.Vector3dVector(np.column_stack(np.column_stack([x.flat, y.flat, depth.flat]) * 3))

    # If you have a color image for the points, assign it here
    # Convert BGR to RGB
    color_img_rgb = cv2.cvtColor(color_img, cv2.COLOR_BGR2RGB)
    pcd.colors = o3d.utility.Vector3dVector(color_img_rgb.reshape(-1, 3) / 255.0)
...
while True:
    ...
    if left_ret and right_ret:
        ...
        arr = np.array(resized_predictions[0])
